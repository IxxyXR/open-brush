...
namespace TiltBrush
{
    ...
    class FlatShapeBrush : GeometryBrush
    {
            ...
            for (int iKnot = iKnot0; iKnot < m_knots.Count; ++iKnot)
            {
                ...
                if (cur.HasGeometry)
                {
                    ...
                    if (isStart)
                    {
                        ...
                        w0 = 0;
                        // TODO remove invalid initial tri
                        MakeFace(ref cur, prev.point.m_Pos, prev.point.m_Pos, halfSize, up, rt, fwd, w0);
                    }
                    else
                    {
                        cur.iVert -= 6;
...

                    {
                        ...
                        MakeFace(ref cur, cur.point.m_Pos, m_knots[iKnot-1].point.m_Pos, halfSize, up, rt, fwd, w1);
                    }
                }
        ...
        void MakeFace(
            ref Knot k, Vector3 center, Vector3 prevCenter, float halfSize,
            Vector3 up, Vector3 rt, Vector3 fwd, float w)
        {
            up *= halfSize;
            rt *= halfSize;
            
            // TODO Maybe sample knots instead of calculating for every knot
            m_centroid = m_knots
                .Aggregate(Vector3.zero, (sum, knot) => sum + knot.point.m_Pos)
                / m_knots.Count;

            // Update the other centroids
            for (var index = 2; index < m_geometry.m_Vertices.Count; index+=3)
            {
                m_geometry.m_Vertices[index] = m_centroid;
            }

            var normal = Vector3.Cross(center - prevCenter, center - m_centroid).normalized;
            
            AppendVert(ref k, center, -normal, rt, 0, 0, w);
            AppendVert(ref k, prevCenter, -normal, rt, 0, 1, w);
            AppendVert(ref k, m_centroid, -normal, rt, 1, 1, w);
